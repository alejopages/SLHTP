...
def generate(ctx):
    ...
    if osp.exists(val_path):
        ...
        if ctx.obj.get('analysis_type') in ['cob', 'ear']:
            columns = ['sample', 'orientation', '1', '2', '3', 'notes', 'dir']
        elif ctx.obj.get('analysis_type') == 'kernel':
            columns = ['sample', 'suffix', 'valid', 'notes', 'dir']
    ...
    for result_dir in os.listdir(path):
        ...
        orientation = [filename.split('_')[1] for filename in filenames]

        a_t = ctx.obj.get('analysis_type')
        if a_t in ['cob', 'ear']:
            curr_report = pd.DataFrame(columns=columns)
            curr_report['sample'] = name
            curr_report['orientation'] = orientation
        elif a_t == 'kernel':
            curr_report = pd.DataFrame(columns=columns)
            curr_report['sample'] = name
            curr_report['group'] = orientation
...
def collate(ctx):
    ...
    log.info("Convert pixels to inches")
    if ctx.obj.get('analysis_type') in ['ear', 'kernel']:
        dpi = 1200
    elif ctx.obj.get('analysis_type') == ['cob']:
        dpi = 300

    for column in results_columns:
        if column not in ['fileName', 'row', 'group', 'analysis_name', 'objectNumber']:
            cumulative[column] = cumulative[column].apply(lambda x: x / dpi)
...
def _produce_kernel_summary_report(data):
    ...
    summary_report['genotype'] = summary_report['fileName'].apply(lambda x: x.split("_")[0])
